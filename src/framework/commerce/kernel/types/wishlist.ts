...
export type WishlistTypes = {
  ...
  wishlistDetail: wishlistDetailBody;
};

export type GetWishlistHook<T extends WishlistTypes = WishlistTypes> = {
  data: T['wishlist'] | null;
  fetchData: { getUserWishlists: T['wishlist'] } | null;
  body: { includeProducts?: boolean };
  input: { includeProducts?: boolean };
  fetcherInput: { customerId: string; includeProducts?: boolean };
  swrState: { isEmpty?: boolean };
};
export type wishlistDetailBody = {
  id: string;
};

export type GetWishlistDetailHook<T extends WishlistTypes = WishlistTypes> = {
  data: T['wishlist'] | null;
  fetchData: { getUserWishlistDetail: T['wishlist'] } | null;
  input: T['wishlistDetail'];
  fetcherInput: T['wishlistDetail'];
  swrState: { isEmpty?: boolean };
};
...
export type UpdateWishlistHook<T extends WishlistTypes = WishlistTypes> = {
  data: T['wishlist'] | null;
  body: { itemId: string };
  fetcherInput: { itemId: string };
  actionInput: { id: string };
